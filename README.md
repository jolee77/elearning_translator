...
### 나레이션 `(한글)` / `(베트남어)` 라벨
- 추출: 스토리보드 하단 나레이션 중 **`(한글)` 표시가 있는 문장만** 번역 대상 (`교수:`/`성우:` 스크립트 제외)
- VN PPTX: `(한글)` 줄은 맞춤법 반영본 유지, **`(베트남어)` 영역**에 번역문 삽입
- `(한글)` 라벨이 없는 기존 스토리보드는 나레이션이 추출되지 않음 — 템플릿에 라벨 추가 후 **다시 추출** 필요

## 최근 수정 (2026-07-27)

### 번역 결과 표 (VN PPTX)
- 헤더가 `# | 번역 대상 텍스트 | 번역결과`인 표는 영역과 무관하게 2열 원문을 번역해 **같은 행 3열**에 기입
- 추출 시에도 해당 표는 2열만 화면텍스트로 포함 (헤더·번호열·빈 결과열 제외)
...
- **나레이션 단일 박스** — ~~스크립트/하단 밴드 통합~~ → **`(한글)` 라벨 문장만** 추출 (`extractHangulNarrationText`), `교수:`/`성우:` 스크립트 제외
...
> 기존 프로젝트는 Step 1 **「다시 추출」** 필요. 나레이션은 스토리보드에 `(한글)`/`(베트남어)` 라벨이 있어야 합니다.
