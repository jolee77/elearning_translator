...
export function UsersPage() {
  ...
  const handleUpdate = async (e: FormEvent) => {
    ...
    if (editingUser.id === currentProfile?.id && editRole !== 'admin') {
      showToast('자신의 관리자 역할은 해제할 수 없습니다.', 'error')
      return
    }
    ...
    try {
      ...
      showToast(getErrorMessage(err, '사용자 정보 수정에 실패했습니다.'), 'error')
    }
  }
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    try {
      ...
      showToast(getErrorMessage(err, '사용자 등록에 실패했습니다.'), 'error')
    }
  }
  ...
  return (
    <div>
      <div className="nb-page-toolbar">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            사용자 등록·조회 및 이름·이메일·비밀번호·역할 수정 (관리자 전용)
          </p>
        </div>
        <button
          type="button"
      ...
      {editingUser && (
        <form onSubmit={handleUpdate} className="nb-card nb-input-surface mb-6 p-6">
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <h3 className="text-sm font-semibold" style={{ color: '#0958d9' }}>
                사용자 정보 수정
              </h3>
              <p className="mt-1 text-xs text-gray-500">
                {editingUser.email}
                {editingUser.id === currentProfile?.id ? ' (나)' : ''}
              </p>
            </div>
            <button type="button" onClick={closeEditForm} className="nb-btn-secondary text-sm">
              닫기
            </button>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="editName" className="nb-field-label">
                이름
              </label>
              <input
                id="editName"
            ...
            <div>
              <label htmlFor="editEmail" className="nb-field-label">
                이메일
              </label>
              <input
                id="editEmail"
            ...
            <div>
              <label htmlFor="editPassword" className="nb-field-label">
                새 비밀번호
              </label>
              <input
                id="editPassword"
            ...
            <div>
              <label htmlFor="editRole" className="nb-field-label">
                역할
              </label>
              <select
                id="editRole"
              ...
              {editingUser.id === currentProfile?.id && (
                <p className="mt-1 text-xs text-gray-400">자신의 역할은 변경할 수 없습니다.</p>
              )}
            </div>
          </div>
          <div className="mt-4 flex justify-end gap-2">
            <button type="button" onClick={closeEditForm} className="nb-btn-secondary">
              취소
            </button>
            <button
              type="submit"
                ...
                <tr key={user.id}>
                  ...
                  <td>
                    <button
                      ...
                      className="text-sm font-medium text-[#0958d9] hover:underline"
                    >
                      수정
                    </button>
                  </td>
