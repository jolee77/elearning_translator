...
- `exclude_from_translation` (boolean, 기본 false) — Step3에서 슬라이드 전체 제외
- `excluded_fields` (text[], 기본 {}) — Step3에서 필드 단위 제외 (`screen_text_*`, `tr_narration`)
...
- 단, 매 화면 반복되는 짧은 구간 라벨(`학습열기`·`학습목표` 등)은 상단 부제목에서만 제외 (본문 섹션 헤더는 유지)
- 동일 슬라이드 내 **동일 문구 중복 박스** 제거
- 읽기 순서: 상단 부제목 → (좌측 세로 스택+우측 텍스트 시) 왼→오 컬럼 → 그 외 위→아래
...
- 위치: 원본 박스와 **동일 x**(좌측 정렬), y + h + 30000 EMU — **그룹(grpSp) 여부와 무관**하게 `chOff`/`chExt`·중첩 스케일을 반영한 **슬라이드 절대 좌표**
- 삽입 위치: 그룹 내부가 아니라 **`spTree` 맨 끝**(최상위 z-order) — 기존 도형·그룹에 가리지 않음
- 크기: 원문이 **한 줄**이면 번역도 한 줄(`wrap=none`)·가로 폭을 번역 길이에 맞게 확장 / 원문에 줄바꿈이 있으면 원본 w 기준 줄바꿈
- 높이: **추정 높이**(줄 수·폰트 기반, cy=0 금지 — 선처럼 보이는 문제 방지), spAutoFit
- 배경: 없음 (투명)
...
### 완료
- [x] Step 4·역번역·엑셀·전문가 링크에서 Step3 제외(슬라이드·필드) 미표시/미포함
- [x] Step 3 제외 저장 시 제외분 기존 번역·역번역 결과 정리
- [x] Step 3에서 슬라이드·텍스트 단위 제외 통합 (번역·전문가 공통), Step 4 제외 UI 제거
